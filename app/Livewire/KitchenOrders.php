...
{    
    ...
    {
        // order_typeがtakeoutの注文のみ取得
        $this->orders = Order::with('items.menu')
            ->where('order_type', 'takeout')
            ->latest()
            ->get();
        // 件数カウント（テイクアウトのみ）
        $this->countAll = Order::where('order_type', 'takeout')->count();
        $this->countNew = Order::where('order_type', 'takeout')->where('status', 'new')->count();
        $this->countPreparing = Order::where('order_type', 'takeout')->where('status', 'preparing')->count();
        $this->countReady = Order::where('order_type', 'takeout')->where('status', 'ready')->count();
        $this->countDelivered = Order::where('order_type', 'takeout')->where('status', 'delivered')->count();
        $this->clock = now()->format('H:i:s');
    }
