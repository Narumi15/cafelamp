<div>
    ...
    <main class="flex-grow container mx-auto p-6">
        ...
        <div class="mb-6 flex flex-wrap gap-3">
            <button class="status-filter px-6 py-3 bg-gray-900 border border-gray-700 rounded-lg" disabled>
                全注文 <span class="ml-2 bg-gray-700 text-white text-xs px-2 py-1 rounded-full">{{ $countAll ?? 0 }}</span>
            </button>
            <button class="status-filter px-6 py-3 bg-gray-900 border border-gray-700 rounded-lg" disabled>
                店内注文 <span class="ml-2 bg-purple-600 text-white text-xs px-2 py-1 rounded-full">{{ \App\Models\Order::where('order_type','dinein')->count() }}</span>
            </button>
            <button class="status-filter px-6 py-3 bg-gray-900 border border-gray-700 rounded-lg" disabled>
                テイクアウト <span class="ml-2 bg-pink-600 text-white text-xs px-2 py-1 rounded-full">{{ \App\Models\Order::where('order_type','takeout')->count() }}</span>
            </button>
        </div>

        <!-- 注文リスト（テイクアウトのみ） -->
        <div class="mb-2 text-sm text-gray-400">テイクアウト注文件数: {{ count($orders) }} / 現在時刻: {{ $clock }}</div>
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" wire:poll.3s="takeout">
                    ...
                    <div class="px-4 py-3 flex justify-between items-center
                        ...
                        <div class="flex items-center">
                            <div class="w-3 h-3 rounded-full mr-2 pulse
                                ...
                                @endif"></div>
                            <h3 class="font-bold">注文番号: {{ $order->id }} <span class="ml-2 text-xs bg-pink-600 text-white px-2 py-1 rounded">テイクアウト</span></h3>
                        </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-grow">
                                        <div class="flex justify-between">
                                            <p class="font-medium">{{ $item->menu->name }} <span class="ml-1 text-xs bg-pink-600 text-white px-1 rounded">テイクアウト</span></p>
                                            <p>×{{ $item->quantity }}</p>
                                        </div>
    ...
    <div id="orderDetailModal" class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
        <div class="bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6 border-b border-gray-700">
                <div class="flex justify-between items-center">
                    <h3 class="text-xl font-bold">注文詳細 - 注文番号: {{ $selectedOrder->id }}</h3>
            ...
            <div class="p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-sm text-gray-400">{{ $selectedOrder->created_at->format('Y/m/d H:i') }}</span>
                </div>
                <div class="space-y-4 mb-6">
                    @foreach($selectedOrder->items as $item)
                        <div class="flex items-center">
                            <div class="w-16 h-16 bg-gray-800 rounded-lg mr-4 flex-shrink-0 overflow-hidden"></div>
                            <div class="flex-grow">
                                <div class="flex justify-between">
                                    <p class="font-medium text-lg">{{ $item->menu->name }}</p>
                                    <p>×{{ $item->quantity }}</p>
                                </div>
                                <p class="text-gray-400">{{ $item->menu->description ?? '' }}</p>
                                <p class="text-gray-400 text-sm mt-1">¥{{ number_format($item->menu->price ?? 0) }} × {{ $item->quantity }} = ¥{{ number_format(($item->menu->price ?? 0) * $item->quantity) }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
                <!-- 金額計算 -->
                @php
                    $subtotal = $selectedOrder->items->sum(function($item) { return ($item->menu->price ?? 0) * $item->quantity; });
                ...
                <div class="border-t border-gray-700 pt-4">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <button class="status-btn active border border-red-500 text-red-400 rounded-lg py-2 text-sm" data-status="new">受注</button>
                        <button class="status-btn border border-gray-600 text-gray-400 rounded-lg py-2 text-sm" data-status="preparing">調理中</button>
                        <button class="status-btn border border-gray-600 text-gray-400 rounded-lg py-2 text-sm" data-status="ready">完成</button>
                    </div>
                </div>
            </div>
        </div>
